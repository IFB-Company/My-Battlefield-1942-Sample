...
namespace GameEventBus
{
    ...
    /// </summary>
    public class EventBus : IEventBus, IInjectable
    {
        ...
        public void Unsubscribe<TEventBase>(Action<TEventBase> action) where TEventBase : EventBase
        {
            ...
            lock (SubscriptionsLock)
            {
                ...
                if (_subscriptions.ContainsKey(type))
                {
                    ...
                    var subscriptionToRemove = allSubscriptions.FirstOrDefault((x) => action.Equals((Action<TEventBase>)x.SubscriptionToken));
        ...
        public void Publish<TEventBase>(TEventBase eventItem) where TEventBase : EventBase
        {
            ...
            for (var i = allSubscriptions.Count - 1; i >= 0; i--)
            {
                allSubscriptions[i].Publish(eventItem);
            }
        }
